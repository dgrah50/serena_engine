...
export const keywords = function(textString) {
  var stopWordsJson = require("../constants/stopwords.json");
  var stopWordsString = Object.values(stopWordsJson).join("\n");
  var tokenizer = new natural.WordTokenizer();
  var text = tokenizer.tokenize(textString).map(function(a) {
  ...
  for (var i = 0; i < text.length; i++) {
    ...
    if (!map[currentWord]) map[currentWord] = newWord();
    for (var j = 1; j <= n && i + j < text.length; j++) {
      // adjacent to the next n words
      var adjacentWord = text[i + j];
      if (!map[adjacentWord]) map[adjacentWord] = newWord();
  ...
  while (scoreChange >= threshold && iters < 200) {
    ...
    for (var key in map) {
      ...
      for (var i = 0; i < map[key].neighbors.length; i++) {
        newMap[key].score +=
          (d * map[map[key].neighbors[i]].score) /
          map[map[key].neighbors[i]].neighbors.length;
      }
    }
  ...
  keywords = keywords.slice(0, Math.floor(numKeywords * 2)).map(function(word) {
    return word.value;
  ...
  for (var i = 0; i < text.length; i++) {
    if (keywords.indexOf(text[i]) != -1) {
      ...
      if (currentStreak.length > 1) {
        ...
        if (
          streaks.indexOf(phrase) >= 0 &&
          multiWordKeyphrases.indexOf(phrase) == -1
        )
          multiWordKeyphrases.push(phrase);
        else streaks.push(phrase);
      }
      currentStreak = [];
    }
  }
  ...
  keywords = keywords.concat(keywords.map(keyword => lemmatizer(keyword)))
  ...
  function wordListToString(list) {
    var result = list[0];
    for (var i = 1; i < list.length; i++) result += " " + list[i];
    return result;
  }
};
